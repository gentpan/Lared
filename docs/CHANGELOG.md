...
### 🐛 修复
- 🔧 修复 PJAX 悬浮预加载与点击导航发送重复请求的问题
  - 鼠标悬浮触发 `prefetchUrl()` 后用户立即点击，会再发一个相同 URL 的请求
  - `prefetchUrl()` 现在将 XHR 引用存入缓存（`{ time, html, pending, xhr }`）
  - `doRequest` 新增 `cached.pending && cached.xhr` 分支，复用进行中的预加载请求
  - 预加载失败时回退到正常请求，不影响可用性
...
### 🐛 修复
- 🔧 修复注销确认弹窗背景几乎全黑的问题
  - 根因：脚本被多次执行导致 IIFE 重复注册 click 监听器，点击时 6 个半透明遮罩叠加 ≈ 不透明黑色
  - 新增 `window.__laredLogoutDialogInit` 防重入守卫
  - `ensureDialog()` 增加 DOM 查重，确保最多只存在一个遮罩元素
- 🔧 修复 xMojipick 表情插件 PJAX 导航后不可用
  - PJAX head switch CSS 正则增加 `xmojipick-` 前缀保留规则
  - head switch 内联脚本更新时保留 `type` 属性（`application/json`）
  - 替换死代码 `initBilibiliEmojis` 为 `window.xmojipick.refresh()`
  - `reinitAfterPjax()` 任务列表新增 xMojipick 刷新调用

### 🎨 改进
- 💅 注销弹窗遮罩新增 `backdrop-filter: blur(4px)` 磨砂玻璃效果
- 💅 xMojipick 插件 PJAX 兼容性重构（主题无关方案）
  - 表情包数据改由 `<head>` 内 `<script type="application/json">` 输出，PJAX head switch 自动更新
  - 从 `wp_localize_script` 中移除 packs 数据，避免重复
  - 新增 `build_js_packs()` 私有方法，提取共享逻辑
  - JS 端 `init()` 自动从 head JSON 元素读取 packs 数据
...
### ✨ 新增
- 🆕 文章卡片悬浮统计面板：触发区域改为整张卡片（`.home-article`），通过 `:has()` 选择器在 hover 分类标签时自动隐藏统计面板
- 🆕 代码块 & 表格三级移动端响应式
  - ≤1024px：代码字号 12.5px，表格字号 13px / 内距 10-12px
  - ≤900px：代码字号 12px，表格字号 12px / 内距 8-10px + `overflow-x: auto`
  - ≤480px：代码字号 11.5px，表格内距 6-8px
- 🆕 评论 OS / 浏览器标签在 ≤720px 时自动隐藏
- 🆕 回到顶部按钮三态响应式：>1280px 固定右下 → 900–1280px 内联在 footer 图标组 → <900px 恢复固定右下

### 🎨 改进
- 💅 响应式断点标准化：非标断点（1664/1500/768/600）统一合并为标准链 1680→1440→1280→1024→900→720→640→480
- 💅 Hero 区域响应式：标题字号梯度缩放、类型徽章 ≤1024px 时 2×2 网格布局
- 💅 Hero 区域在 ≤1024px 时完全隐藏，清理所有移动端 Hero 冗余 CSS
- 💅 全局 ≤1024px 侧边距 20px（wrapper / header / footer）
- 💅 侧栏 Tab 样式重构：默认 400 字重，激活态红色背景 + 白色文字 + 700 字重，编号图标 1–5 绝对定位
- 💅 侧栏宽度在 1440px 以下全部断点最小值 200px
- 💅 统计面板展开动画：从右弹出改为向左展开（expand-left）
- 💅 文章悬浮统计图标更换：`fa-solid fa-fire` + `fa-regular fa-comment`
- 💅 Footer 图标组：≤1280px 靠右对齐，≤900px 居中对齐
- 💅 NEW 徽章仅在首页第一页显示（`1 === $paged` 条件判断）
- 💅 标题破折号编码修复：`wp_specialchars_decode()` → `html_entity_decode()`，正确处理 `&#8211;` 等数字实体（index.php 8 处 + inc-hero.php 2 处）
- 💅 回到顶部按钮移入 `.site-footer-icons` 容器统一管理

### 🗑️ 移除
- ❌ 删除 `inc/inc-mshots.php`（功能已合并至 `inc/inc-image.php`）
- ❌ 删除孤儿文件 `data/feed-data.json`（活跃缓存位于 `data/rss/feed-data.json`）
