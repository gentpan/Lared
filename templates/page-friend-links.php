...
if (have_posts()) :
    while (have_posts()) :
        ...
        $random_bookmark_url = '';
        if ($bookmark_count > 0) {
            $random_index = array_rand($bookmarks);
            $random_bookmark = $bookmarks[$random_index] ?? null;
            if ($random_bookmark instanceof stdClass && !empty($random_bookmark->link_url)) {
                $random_bookmark_url = (string) $random_bookmark->link_url;
            }
        }
        ...
        <main class="main-shell mx-auto w-full max-w-[1400px] min-h-[calc(100vh-64px)] border-x border-[#d9d9d9] bg-[#ffffff] pb-[90px] max-[900px]:pb-16">
                ...
                <div class="listing-head-inner">
                    ...
                    <div class="listing-head-main">
                        <div class="listing-head-title-row">
                            <h1 class="listing-head-title friend-links-head-title"><i class="fa-sharp fa-thin fa-square-share-nodes" aria-hidden="true"></i><?php the_title(); ?></h1>
                            <div class="friend-links-head-side">
                                <?php if ('' !== $random_bookmark_url) : ?>
                                    <a class="friend-links-random-btn" href="<?php echo esc_url($random_bookmark_url); ?>" rel="noopener noreferrer" data-random-delay="1000">
                                        <i class="fa-solid fa-dice" aria-hidden="true"></i>
                                        <?php esc_html_e('随机访问', 'lared'); ?>
                                    </a>
                                <?php endif; ?>
                            </div>
                        </div>
                            ...
                            <?php if ($is_text_style) : ?>
                                ...
                                <div class="friend-links-text-grid">
                                    <?php foreach ($cat_bookmarks as $bookmark) :
                                        ...
                                        // 优先使用自定义图片，没有则从本地缓存获取 favicon
                                        $avatar_url = lared_extract_bookmark_avatar_url($bookmark->link_image ?? '');
                                        if ('' === $avatar_url && !empty($host)) {
                                            $avatar_url = lared_get_cached_link_icon($host);
                                        }
                                    ?>
                                        <a class="friend-link-text-item" href="<?php echo esc_url($site_url); ?>" target="_blank" rel="noopener noreferrer" data-mshot-url="<?php echo esc_attr(lared_get_mshot_url($site_url)); ?>">
                                            <?php if ('' !== $avatar_url) : ?>
                                ...
                                <div class="friend-links-grid">
                                    <?php foreach ($cat_bookmarks as $bookmark) : ?>
                                        ...
                                        <article class="friend-link-card">
                                            <a class="friend-link-card-link" href="<?php echo esc_url($site_url); ?>" target="_blank" rel="noopener noreferrer" data-mshot-url="<?php echo esc_attr(lared_get_mshot_url($site_url)); ?>">
                                                <div class="friend-link-card-avatar">
                                                    <?php if ('' !== $avatar_url) : ?>
                                                        <img class="friend-link-card-avatar-img" src="<?php echo esc_url($avatar_url); ?>" alt="<?php echo esc_attr($site_name); ?>" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';" />
                                                        <span class="friend-link-card-avatar-letter" style="display:none;" aria-hidden="true"><?php echo esc_html(mb_substr($site_name, 0, 1)); ?></span>
                                                    <?php else : ?>
                                                        <span class="friend-link-card-avatar-letter" aria-hidden="true"><?php echo esc_html(mb_substr($site_name, 0, 1)); ?></span>
                                                    <?php endif; ?>
                                                </div>
            ...
            <script>
                (function() {
                    ...
                    // ── 申请友链模态窗 ──
                    var randomVisitButton = document.querySelector('.friend-links-random-btn[data-random-delay]');
                    if (randomVisitButton) {
                        randomVisitButton.addEventListener('click', function(e) {
                            var targetUrl = randomVisitButton.getAttribute('href');
                            var delay = parseInt(randomVisitButton.getAttribute('data-random-delay'), 10) || 1000;

                            if (!targetUrl || randomVisitButton.classList.contains('is-spinning')) {
                                e.preventDefault();
                                return;
                            }

                            e.preventDefault();
                            randomVisitButton.classList.add('is-spinning');
                            randomVisitButton.setAttribute('aria-disabled', 'true');
