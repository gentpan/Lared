...
if (have_posts()) :
    while (have_posts()) :
        ...
        <main class="main-shell mx-auto w-full max-w-[1400px] min-h-[calc(100vh-64px)] border-x border-[#d9d9d9] bg-[#fff] pb-[90px] max-[900px]:pb-16">
                ...
                <div class="memos-layout">
                    ...
                    <?php if (!empty($items)) : ?>
                        ...
                        <div class="memos-grid" id="memos-grid">
                            <?php foreach ($items as $item) : ?>
                                ...
                                $time_human = $time_source > 0
                                    ...
                                    : '';
                                $hidden_class = $memo_index >= 10 ? ' memos-card--hidden' : '';
                                ?>
                                <article class="memos-card<?php echo $hidden_class; ?>" data-memo-index="<?php echo $memo_index; ?>">
                        ...
                        <?php if (count($items) > 10) : ?>
                            <div class="memos-load-more-wrap" id="memos-load-more-wrap" style="display:none;">
                                <button type="button" class="memos-load-more-btn" id="memos-load-more-btn">
                                    <i class="fa-solid fa-angles-down" aria-hidden="true"></i>
                                    <?php esc_html_e('加载更多', 'lared'); ?>
                                    <span class="memos-load-more-count" id="memos-load-more-count"></span>
                                </button>
                            </div>
                        <?php endif; ?>
        ...
        // 渐进加载：初始10条，滚动+10，按钮每次+20
        (function() {
            var INITIAL = 10;
            var SCROLL_BATCH = 10;
            var BUTTON_BATCH = 20;
            var allCards = document.querySelectorAll('.memos-card[data-memo-index]');
            var total = allCards.length;
            if (total <= INITIAL) return;

            var shown = INITIAL;
            var scrollLoaded = false;
            var loadMoreWrap = document.getElementById('memos-load-more-wrap');
            var loadMoreBtn = document.getElementById('memos-load-more-btn');
            var loadMoreCount = document.getElementById('memos-load-more-count');

            function updateCount() {
                if (!loadMoreCount) return;
                var remaining = total - shown;
                loadMoreCount.textContent = '(' + remaining + ')';
            }

            function showCards(count) {
                var end = Math.min(shown + count, total);
                for (var i = shown; i < end; i++) {
                    allCards[i].classList.remove('memos-card--hidden');
                }
                shown = end;
                updateCount();
                if (shown >= total && loadMoreWrap) {
            ...
            function onScroll() {
                ...
                var grid = document.getElementById('memos-grid');
                if (!grid) return;
                var rect = grid.getBoundingClientRect();
                if (rect.bottom - window.innerHeight < 300) {
                    scrollLoaded = true;
                    showCards(SCROLL_BATCH);
                    if (shown < total && loadMoreWrap) {
