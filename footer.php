            ...
            <?php else : ?>
                <div class="footer-login-wrapper relative">
                    ...
                    <div class="footer-login-dropdown hidden absolute right-0 bottom-full mb-2 w-[280px] rounded-lg border border-[#d9d9d9] bg-white p-5 shadow-lg" data-login-dropdown>
                        <h4 class="mb-4 text-[16px] font-semibold text-[var(--color-title)]"><?php esc_html_e('管理员登录', 'pan'); ?></h4>
                        <form action="<?php echo esc_url(site_url('wp-login.php', 'login_post')); ?>" method="post">
                            <div class="mb-3">
                                <label for="footer-login-user" class="mb-1 block text-[12px] font-medium text-[var(--color-body)]"><?php esc_html_e('用户名', 'pan'); ?></label>
                            ...
                            <div class="mb-4">
                                <label for="footer-login-pass" class="mb-1 block text-[12px] font-medium text-[var(--color-body)]"><?php esc_html_e('密码', 'pan'); ?></label>
...
<div id="pan-aplayer" aria-label="Music Player"></div>
...
<!-- Search Modal -->
<div class="search-modal" data-search-modal aria-hidden="true" role="dialog" aria-label="<?php esc_attr_e('Search', 'pan'); ?>">
    ...
    <div class="search-modal-container">
        <div class="search-modal-header">
            <form role="search" method="get" class="search-modal-form" action="<?php echo esc_url(home_url('/')); ?>">
                ...
                <label class="sr-only" for="search-modal-input"><?php esc_html_e('Search for:', 'pan'); ?></label>
                <input id="search-modal-input" type="search" name="s" placeholder="<?php esc_attr_e('搜索文章...', 'pan'); ?>" class="search-modal-input" autocomplete="off" />
                <kbd class="search-modal-esc">ESC</kbd>
            </form>
        </div>
        <div class="search-modal-body" data-search-results>
            <div class="search-modal-hint">
                <p><?php esc_html_e('输入关键词搜索文章', 'pan'); ?></p>
            </div>
        </div>
    </div>
</div>
