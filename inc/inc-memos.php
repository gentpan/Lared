...
function lared_memos_strip_hashtag_markers(string $content): string
{
    ...
    // 完全移除 #关键词（关键词已在徽章中显示，正文不再重复）
    $stripped = preg_replace('/[#＃]([\p{L}\p{N}_\-]{1,32})/u', '', $content);
...
function lared_memos_process_images(string $content): string
{
    ...
    $content = preg_replace_callback(
        ...
        function ($matches) use ($site_url) {
            ...
            // 处理相对路径
            if ($site_url && !str_starts_with($url, 'http') && !str_starts_with($url, '//')) {
...
/**
 * 从 Memos API 的 attachments/resources 字段中提取图片 HTML
 * 
 * 最新版 Memos (v0.24+) 使用 attachments 字段：
 *   { "name": "attachments/xxx", "filename": "img.png", "type": "image/png", "externalLink": "" }
 *   图片 URL = {site_url}/file/{name}/{filename}
 *
 * 旧版 Memos (v0.22) 使用 resources 或 resourceList 字段。
 *
 * @param array $memo 单条 memo 数据
 * @return string 图片 HTML（可能为空）
 */
function lared_memos_build_resource_images(array $memo): string
{
    // 按优先级尝试：attachments（最新版）> resources > resourceList（旧版）
    $items = $memo['attachments'] ?? $memo['resources'] ?? $memo['resourceList'] ?? [];
    ...
    foreach ($items as $res) {
        ...
        $type = (string) ($res['type'] ?? $res['mimeType'] ?? '');
        // 只处理图片类型的资源
        ...
        $external_link = (string) ($res['externalLink'] ?? '');
        $res_uid       = (string) ($res['uid'] ?? '');
        $res_id        = (string) ($res['id'] ?? '');

        // 1) 优先使用 externalLink（外部存储/S3 链接）
        if ('' !== $external_link) {
            $url = $external_link;
        }
        // 2) 最新版：name 形如 "attachments/xxx"，URL = /file/{name}/{filename}
        elseif ('' !== $res_name && '' !== $filename && $site_url) {
            $url = untrailingslashit($site_url) . '/file/' . $res_name . '/' . rawurlencode($filename);
        }
        // 3) 旧版 v0.22：name 形如 "resources/xxx"
        elseif ('' !== $res_name && '' !== $filename && $site_url && str_starts_with($res_name, 'resources/')) {
            $url = untrailingslashit($site_url) . '/file/' . $res_name . '/' . rawurlencode($filename);
        }
        // 4) 旧版备用：uid + filename
        elseif ('' !== $res_uid && '' !== $filename && $site_url) {
            $url = untrailingslashit($site_url) . '/file/resources/' . $res_uid . '/' . rawurlencode($filename);
        }
        // 5) 更旧版本：/o/r/{id}/{filename}
        elseif ('' !== $res_id && '' !== $filename && $site_url) {
            $url = untrailingslashit($site_url) . '/o/r/' . $res_id . '/' . rawurlencode($filename);
        }
        // 6) 最旧版本兜底：/o/r/{id}
        elseif ('' !== $res_id && $site_url) {
            $url = untrailingslashit($site_url) . '/o/r/' . $res_id;
        }

        if ('' !== $url) {
            $alt = esc_attr($filename);
            $images_html .= '<img class="lazyload" data-src="' . esc_url($url) . '" alt="' . $alt . '" />';
        }
    }

    if ('' !== $images_html) {
        $count = substr_count($images_html, '<img ');
        $grid_class = $count === 1 ? 'memos-images memos-images--single' : 'memos-images memos-images--grid';
        $images_html = '<div class="' . $grid_class . '">' . $images_html . '</div>';
    }
...
// PHP 8.1+ 原生支持 array_is_list()，不再需要 polyfill
...
function lared_get_memos_stream(array $args = []): array
{
    ...
    $cache_key = 'lared_memos_' . md5($api_url . '|' . $token . '|' . $site_url . '|v3_all');
    ...
    $raw_items = [];
    $page_token = '';
    $max_pages = 200; // 安全上限，防止无限循环

    for ($page = 0; $page < $max_pages; $page++) {
        $query_args = ['pageSize' => 500];
        if ('' !== $page_token) {
        ...
        if (isset($decoded['memos']) && is_array($decoded['memos'])) {
            $page_items = $decoded['memos'];
        } elseif (isset($decoded['data']) && is_array($decoded['data'])) {
            $page_items = $decoded['data'];
        } elseif (array_is_list($decoded)) {
            $page_items = $decoded;
        }

        $raw_items = array_merge($raw_items, $page_items);

        // 检查是否有下一页
        $next_token = (string) ($decoded['nextPageToken'] ?? '');
        if ('' === $next_token || empty($page_items)) {
            break;
        }
    ...
    foreach ($raw_items as $memo) {
        ...
        $has_resources = !empty($memo['attachments']) || !empty($memo['resources']) || !empty($memo['resourceList']);
        if ('' === $content && !$has_resources) {
            continue;
        }

        $name = (string) ($memo['name'] ?? '');
        $uid = (string) ($memo['uid'] ?? $memo['id'] ?? '');
        $memo_id = '' !== $uid ? $uid : ('' !== $name ? $name : md5($content ?: 'img-only'));
        ...
        $content_html = '' !== $content ? lared_memos_content_to_html($content) : '';
        $content_html .= lared_memos_build_resource_images($memo);
...
function lared_get_memos_cache_dir(): string
{
    ...
    return $upload_dir['basedir'] . '/lared-cache/memos';
}
...
function lared_update_memos_json_cache(): void
{
    ...
    $cache_dir = lared_get_memos_cache_dir();
    if (!is_dir($cache_dir)) {
        wp_mkdir_p($cache_dir);
    }
    $cache_file = lared_get_memos_json_cache_file();
    file_put_contents($cache_file, json_encode($cache_data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT), LOCK_EX);
}
...
function lared_schedule_memos_cache_refresh(): void
{
    ...
    wp_schedule_event(time(), 'daily', 'lared_memos_json_cache_refresh');
}
 ...
 * 检测并修复因时区偏移导致的错误调度时间
 */
function lared_fix_memos_schedule_if_broken(): void
{
    $next = wp_next_scheduled('lared_memos_json_cache_refresh');
    if (false === $next) {
        wp_schedule_event(time(), 'daily', 'lared_memos_json_cache_refresh');
        return;
    }
    // 若下次执行时间超过 25 小时，说明旧调度用了本地时间偏移，重置为立即执行
    if (($next - time()) > 25 * HOUR_IN_SECONDS) {
        wp_unschedule_event($next, 'lared_memos_json_cache_refresh');
        wp_schedule_event(time(), 'daily', 'lared_memos_json_cache_refresh');
    }
}
add_action('wp', 'lared_fix_memos_schedule_if_broken');
...
function lared_render_memo_card(array $item): void
{
    ...
    $time_human = $time_source > 0
        ? sprintf(
            ...
            __('%s前', 'lared'),
            human_time_diff($time_source, time())
        )
        : '';
    ?>
