...
function lared_extract_bookmark_avatar_url(mixed $value): string
{
...
function lared_get_rss_cache_dir(): string
{
    ...
    return trailingslashit($upload_dir['basedir'] . '/lared-cache/rss');
}
...
function lared_clear_rss_cache_files(): array
{
    ...
    // 1. 清除单源缓存 data/rss/*.json（不含 feed-data.json）
    if (is_dir($cache_dir)) {
        ...
        if (is_array($files)) {
            foreach ($files as $file) {
                ...
                // 跳过聚合缓存文件，下面单独处理
                if (basename($file) === 'feed-data.json') {
                    continue;
                }
                if (@unlink($file)) {
                    $removed++;
    ...
    // 2. 清除友联动态聚合缓存 feed-data.json
    $feed_json = lared_get_feed_json_cache_file();
    if (is_file($feed_json)) {
        if (@unlink($feed_json)) {
            $removed++;
        } else {
            $errors++;
...
function lared_get_friend_rss_sources(): array
{
    ...
    foreach ($bookmarks as $bookmark) {
        $site_url = esc_url_raw((string) ($bookmark->link_url ?? ''));
        if ('' === $site_url) {
            continue;
        }
        ...
        $site_name = sanitize_text_field((string) ($bookmark->link_name ?? ''));
        $site_desc = sanitize_text_field((string) ($bookmark->link_description ?? ''));
        $site_avatar = lared_extract_bookmark_avatar_url($bookmark->link_image ?? '');
        $host = (string) wp_parse_url($site_url, PHP_URL_HOST);

        $sources[] = [
            'bookmark_id' => (int) ($bookmark->link_id ?? 0),
...
function lared_fetch_source_feed(array $source): array
{
    ...
    // 将 SimplePie 内置缓存 TTL 缩短到 5 小时 50 分钟，确保每次 6 小时 cron 触发时
    // SimplePie transient 已过期，能真正从远端拉取最新数据而不是返回旧缓存。
    $ttl_override = static function (): int {
        return 5 * HOUR_IN_SECONDS + 50 * MINUTE_IN_SECONDS;
    };
    add_filter('wp_feed_cache_transient_lifetime', $ttl_override);
    ...
    remove_filter('wp_feed_cache_transient_lifetime', $ttl_override);
...
{
    $upload_dir = wp_upload_dir();
    return $upload_dir['basedir'] . '/lared-cache/rss';
}
...
/**
 * 设置友联动态缓存定时任务（每 6 小时）
 * wp_schedule_event 需要 UTC 时间戳，直接使用 time() 立即起跑，
 * 此后每 6 小时自动续排，无时区偏移问题。
 */
function lared_schedule_feed_cache_refresh(): void
{
    ...
    wp_schedule_event(time(), 'lared_four_times_daily', 'lared_feed_json_cache_refresh');
}
// 调度检测已移至 lared_fix_feed_schedule_if_broken()
...
/**
 * 修正旧版本中可能存在的错误调度（时区偏移导致时间不对）
 * 在 wp 钩子上检测：若已有调度但调度时间异常（距今超过 12 小时），则重排。
 */
function lared_fix_feed_schedule_if_broken(): void
{
    $next = wp_next_scheduled('lared_feed_json_cache_refresh');
    if (false === $next) {
        // 未调度，直接注册
        wp_schedule_event(time(), 'lared_four_times_daily', 'lared_feed_json_cache_refresh');
        return;
    ...
    // 若下次执行时间距现在超过 7 小时，说明旧调度时间有问题，重排
    if (($next - time()) > 7 * HOUR_IN_SECONDS) {
        wp_unschedule_event($next, 'lared_feed_json_cache_refresh');
        wp_schedule_event(time(), 'lared_four_times_daily', 'lared_feed_json_cache_refresh');
    }
}
add_action('wp', 'lared_fix_feed_schedule_if_broken');
...
function lared_ajax_save_feed_cache(): void
{
    ...
    $raw = wp_unslash($_POST['feed_data'] ?? '');
...
function lared_render_feed_card(array $item): void
{
    ...
    $time_text = '';
    if ($item_timestamp > 0) {
        $time_text = sprintf(__('%s前', 'lared'), human_time_diff($item_timestamp, time()));
    }
?>
